...
function MenuEditor(idSelector, options) {
    ...
    function TButtonGroup() {
        ...
        var $btnOut = TButton({classCss: 'btn btn-secondary btn-sm btnOut btnMove', text: '<i class="fas fa-angle-left clickable"></i>'});
        var $btnIn = TButton({classCss: 'btn btn-secondary btn-sm btnIn btnMove', text: '<i class="fas fa-angle-right clickable"></i>'});
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
			...
			//itemObject.texten = "";
			var $textItem = '';
			$.each( settings.elementsToShow, function( element ) {
				$textItem += v[settings.elementsToShow[element]] + '&nbsp;';
			});
			
			$div.append($('<span>').addClass('txt').append($textItem).css('margin-right', '5px'));

			//$div.append($i).append($divbtn);
			$div.append($divbtn);
	...
	function _updateSelectedType() {
		...
		$('.module_dropdown').addClass('d-none');
    ...
    this.update = function(){
		...
		var textItem = '';
		$.each( settings.elementsToShow, function( element ) {
		  textItem = textItem + '' + $cEl.data(settings.elementsToShow[element]) + ' ';
		});
		
		//$cEl.find('span.txt').first().text($cEl.data('text'));
		$cEl.find('span.txt').first().text(textItem);
    ...
    this.add = function(){
		...
		textItem = '<span class="txt">';
		$.each( settings.elementsToShow, function( element ) {
		  textItem = textItem + data[settings.elementsToShow[element]] + ' ';
		});
		textItem = textItem + '</span>';
