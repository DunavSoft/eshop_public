...
<script>
	$(function() {
		...
		const baseUrl = '<?= site_url($locale . '/admin/pages/' . $page) ?>';

		const baseUrlDeleted = '<?= site_url($locale . '/admin/pages/deleted/' . $page) ?>';

		const baseSearchUrl = '<?= site_url($locale . '/admin/pages/search/' . $page) ?>';

		const baseSearchUrlDeleted = '<?= site_url($locale . '/admin/pages/deleted/search/' . $page) ?>';

		let urlListString = `${baseUrl}`;

		let urlListStringDeleted = `${baseUrlDeleted}`;

		let urlListSearchString = `${baseSearchUrl}`;

		let urlListSearchStringDeleted = `${baseSearchUrlDeleted}`;
		...
		function search_ajax() {
			...
			var ajaxUrl = deleted ? urlListSearchStringDeleted : urlListSearchString;
		...
		$topSearchText.keyup(function(e) {
			...
			if (strlen == 0 && deleted == 0) {
				retrieveAjaxListData(urlListString);
			}

			if (strlen == 0 && deleted == 1) {
				retrieveAjaxListData(urlListStringDeleted);
			}
		...
		$(document).on('click', '.edit-button', function(event) {
			...
			if (lastEditedId == id && lastEditedId != 'new') {
				console.log('pages_js - return from edit-button');
		...
		$(document).ready(function() {
			$("#ModalFormSecondary").on('hide.bs.modal', function() {
				...
				var urlDeleted = new URL(urlListStringDeleted);
		...
		<?php if (!$isAjaxRequest) : ?>
			$(document).ready(function() {
				retrieveAjaxFormData('<?= site_url($locale . '/admin/pages/form/' . $id . '/' . $duplicate) ?>', '<?= $id ?>');
		...
		$(document).on('click', '.submit-button', function(event) {
			...
			$.post(ajaxUrl, form_data, function(response) {
				...
				if (response.status == 'success') {
					...
					// Populate the slug fields with the response data
					for (var lang_id in response.data.slugs) {
						$('#slug' + lang_id).val(response.data.slugs[lang_id]);
					}
				...
				$('#form').attr('action', '<?= site_url($locale . '/admin/pages/form_submit') ?>/' + response.data.id);

				//change the url string
				var url = new URL('<?= site_url($locale . '/admin/pages/form') ?>/' + response.data.id);
