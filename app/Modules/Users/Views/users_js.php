<script>
  $(function() {
    let lastEditedId = 0;
    const baseUrl = '<?= site_url($locale . '/admin/users') ?>';
    ...
    function search_ajax() {
      ...
      $.post('<?= site_url($locale . '/admin/users/search') ?>', form_data, function(response) {
        $('#ajax-content').html(response);
      });
    }
    ...
    $(document).on('click', '.edit-button', function(event) {
      ...
      if (lastEditedId == id && lastEditedId != 'new') {
        console.log('users_js - return from edit-button');
    ...
    <?php if (!$isAjaxRequest) : ?>
      $(document).ready(function() {
        retrieveAjaxFormData('<?= site_url($locale . '/admin/users/form/' . $id . '/' . $duplicate) ?>', '<?= $id ?>');
    ...
    $(document).on('click', '.submit-button', function() {
      ...
      $.post(ajaxUrl, form_data, function(response) {
        ...
        $('#form').attr('action', '<?= site_url($locale . '/admin/users/form_submit') ?>/' + response.data.id);

        //change the url string
        var url = new URL('<?= site_url($locale . '/admin/users/form') ?>/' + response.data.id);
