<script>
	...
	var urlListString = '<?= site_url($locale . '/admin/colors/' . $page) ?>';
	...
	$(document).on('click', '.edit-button', function(event) {
		...
		if (lastEditedId == id && lastEditedId != 'new') {
			console.log('colors_js - return from edit-button');
	...
	<?php if ($isNotAjaxRequest) : ?>
		$(document).ready(function() {
			retrieveAjaxFormData('<?= site_url($locale . '/admin/colors/form/' . $id . '/' . $duplicate) ?>', '<?= $id ?>');
			$("#ModalFormSecondary").modal('show');

			retrieveAjaxListData('<?= site_url($locale . '/admin/colors/' . $page) ?>');
		});
	<?php endif; ?>
	...
	$(document).on('click', '.submit-button', function() {
		...
		$.post(ajaxUrl, form_data, function(response) {
			...
			$('#form').attr('action', '<?= site_url($locale . '/admin/colors/form_submit') ?>/' + response.data.id);

			//change the url string
			var url = new URL('<?= site_url($locale . '/admin/colors/form') ?>/' + response.data.id);
	...
	function search_ajax() {
		...
		$.post('<?= site_url($locale . '/admin/colors/search') ?>', form_data, function(response) {
			$('#ajax-content').html(response);
		});
	}
</script>