...
class Slug
{
	...
	private function _check_uri($uri, $id = FALSE, $count = 0)
	{
		...
		// Setup the query
    if ($this->checkFieldDeletedAtExists()) {
      $this->qb->select($this->field)->where($this->field, $new_uri)->where('deleted_at', null);
    } else {
  ...
  /**
	 ...
	 * @return string
	 */
  private function checkFieldDeletedAtExists() {
    // Get the list of fields in the table
    $fields = $this->db->getFieldData($this->table);
    
    // Go through each field
    foreach ($fields as $field) {
        // If 'deleted_at' field is found, return true
        if ($field->name == 'deleted_at') {
            return true;
        }
    }

    // If the loop completes without finding the 'deleted_at' field, return false
    return false;
  }
